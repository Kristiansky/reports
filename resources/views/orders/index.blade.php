...
{{--                                        @if($order->incompleta == 'da')--}}
                                        @if($order->getRawOriginal('status') == 'Comanda' || $order->getRawOriginal('status') == 'Blocata')
                                            <form method="post" action="{{route('order.exclude')}}">
                                                ...
                                                @method('POST')
                                                <input type="hidden" name="idcomanda" value="{{$order->idcomanda}}">
                                                <button class="btn btn-warning btn-xs float-left mr-1" onclick="return confirm('{{__('main.are_you_sure')}}')"><i class="fas fa-file-archive" data-tooltip title="{{__('main.exclude')}}"></i></button>
                                            </form>
                                        @endif
{{--                                        @endif--}}
